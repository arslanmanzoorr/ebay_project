...
export default function SuperAdminUsersPage() {
    ...
    const [newUserForm, setNewUserForm] = useState({
        ...
        email: '',
        password: '',
        role: 'admin', // Default to admin for super admin creation? Or let them choose.
        isActive: true,
        updatedAt: new Date()
    });
    ...
    const refreshUserList = async () => {
        ...
        try {
            ...
            // API /api/users returns an array directly based on index.ts code seen earlier
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {users.map((userItem) => (
                                        <tr key={userItem.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <Badge className={
                                                    userItem.role === 'super_admin' ? 'bg-purple-800 text-white' :
                                                        userItem.role === 'admin' ? 'bg-red-100 text-red-800' :
                                                            userItem.role === 'researcher' ? 'bg-blue-100 text-blue-800' :
                                                                userItem.role === 'researcher2' ? 'bg-orange-100 text-orange-800' :
                                                                    'bg-gray-100 text-gray-800'
                                                }>
                                                    {userItem.role ? (userItem.role.charAt(0).toUpperCase() + userItem.role.slice(1)).replace('_', ' ') : 'Unknown'}
                                                </Badge>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                    ...
                                                    <TooltipProvider>
                                                        <Tooltip>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="sm" onClick={() => openEditUser(userItem)}>
